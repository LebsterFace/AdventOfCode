...
const getPaths = (node, path = [], double = null) => {
	...
	for (const nextNode of connectedNodes) {
		let attemptedDouble = double;
		const isSmallCave = nextNode.toLowerCase() === nextNode;

		if (isSmallCave && path.includes(nextNode)) {
			if (nextNode !== "start" && nextNode !== "end" && attemptedDouble === null) {
				attemptedDouble = nextNode;
			} else {
				continue;
			}
		}

		results.push(getPaths(nextNode, path.concat(node), attemptedDouble));
	}
