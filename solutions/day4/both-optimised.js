	...
	boards: for (let boardNum = boards.length - 1; boardNum >= 0; boardNum--) {
		...
		for (const row of board) {
			for (let i = 0; i < row.length; i++) {
				if (row[i] !== number) continue; // If the current cell is not the number, move to the next cell
				row[i] = -1; // If it is, mark it

				const hasWon = row.every(column => column < 0) || board.every(row => row[i] < 0);
				if (!hasWon) continue; // If the board has not won, move to the next cell
